...
func (api *API) TraceMultiCall(ctx context.Context, args1 []ethapi.CallArgs, blockNrOrHash rpc.BlockNumberOrHash, config *TraceCallConfig) (interface{}, error) {
	// Try to retrieve the specified block
	var (
		err   error
		block *types.Block
	...
	// Execute all transactions except last
	vmctx := core.NewEVMBlockContext(block.Header(), api.chainContext(ctx), nil)
	for _, argTxn := range args1[:len(args1)-1] {
		msg := argTxn.ToMessage(api.backend.RPCGasCap())
		api.traceTxDisabled(ctx, msg, new(Context), vmctx, statedb, traceConfig)
		statedb.Finalise(api.backend.ChainConfig().IsEIP158(block.Number()))
	}

	// Execute trace and return results for last transaction
	argTxn := args1[len(args1)-1]
	msg := argTxn.ToMessage(api.backend.RPCGasCap())
	return api.traceTx(ctx, msg, new(Context), vmctx, statedb, traceConfig)
...
func (api *API) traceTxDisabled(ctx context.Context, message core.Message, txctx *Context, vmctx vm.BlockContext, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	// Assemble the structured logger or the JavaScript tracer
	var (
		tracer    vm.EVMLogger
		err       error
