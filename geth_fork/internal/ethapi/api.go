...
//Stolen from here: https://github.com/ethereum/go-ethereum/pull/22202/commits/9199d2e13d484df7a634fad12343ed2b46d5d4c3
func (s *PublicBlockChainAPI) CallBundle(ctx context.Context, encodedTxs []hexutil.Bytes, blockNrOrHash rpc.BlockNumberOrHash, blockTimestamp *uint64) ([]hexutil.Bytes, error) {
	if len(encodedTxs) == 0 {
		return nil, nil
	}
...
func (s *PublicBlockChainAPI) CallBundleFast(ctx context.Context, encodedTxs []hexutil.Bytes, blockNrOrHash rpc.BlockNumberOrHash, blockTimestamp *uint64) (hexutil.Bytes, error) {
	if len(encodedTxs) == 0 {
		return nil, nil
	}
	var txs types.Transactions
	...
	//Save results only for the last txn
	tx := txs[len(txs)-1]
	msg, err := tx.AsMessage(signer)
	result, err := core.ApplyMessage(evm, msg, gp)
	if err := vmError(); err != nil {
